...
#define JSONXX_ASSERT(...)                                                                                             \
    do {                                                                                                               \
        if (jsonxx::Assertions) jsonxx::assertion(__FILE__, __LINE__, #__VA_ARGS__, bool(__VA_ARGS__));                \
    } while (0)
#endif
...
inline bool parser_is_permissive()
{
    return Parser == Permissive;
}
inline bool unquoted_keys_are_enabled()
{
...
public:
    ...
    std::string xml(unsigned format = JSONx, const std::string &header = std::string(),
...
#undef $number
    ...
    void import(const Value &other)
    {
        if (this != &other) switch (other.type_) {
                case NULL_: import(Null()); break;
                case BOOL_: import(other.bool_value_); break;
                case NUMBER_:
                    import(other.number_value_);
                    precision_ = other.precision_;
                    break;
                case STRING_: import(*other.string_value_); break;
                case ARRAY_: import(*other.array_value_); break;
                case OBJECT_: import(*other.object_value_); break;
                case INVALID_: type_ = INVALID_; break;
                default: JSONXX_ASSERT(!"not implemented");
            }
    }
...
template <> inline bool Value::is<Value>() const
{
    return true;
}
...
template <> inline Value &Value::get<Value>()
{
    return *this;
}

template <> inline const Value &Value::get<Value>() const
{
    return *this;
}
