...
void HumdrumTie::setStart(const string &id, Measure *starting, int layer,
                          const string &token, int pitch, HumNum starttime,
                          HumNum endtime) {
  m_starttoken = token;
...
void HumdrumTie::setEnd(const string &id, Measure *ending,
                        const string &token) {
  m_endtoken = token;
  m_endmeasure = ending;
  m_endid = id;
...
void HumdrumTie::setEndAndInsert(const string &id, Measure *ending,
                                 const string &token) {
...
void HumdrumInput::createHeader(void) {
  ...
  p1.append_child(pugi::node_pcdata)
      .set_value(StringFormat("Transcoded from Humdrum with Verovio version %s",
                              GetVersion().c_str()).c_str());
...
string HumdrumInput::getReferenceValue(const string &key,
                                       vector<HumdrumLine *> &references) {
...
string HumdrumInput::getDateString(void) {
  ...
  string dateStr = StringFormat(
      "%d-%02d-%02d %02d:%02d:%02d", now->tm_year + 1900, now->tm_mon + 1,
...
void HumdrumInput::insertRespStmt(pugi::xml_node &titleStmt,
                                  vector<vector<string> > &respPeople) {
  if (respPeople.size() == 0) {
    return;
  }
  ...
  for (int i = 0; i < (int)respPeople.size(); i++) {
    ...
    person.append_attribute("role") =
        unescapeHtmlEntities(respPeople[i][1]).c_str();
...
void HumdrumInput::getRespPeople(vector<vector<string> > &respPeople,
                                 vector<HumdrumLine *> &references) {
  ...
  addPerson(respPeople, references, "MPN",
            "Performer"); // prf, also: Singer/Instrumentalist
...
void HumdrumInput::addPerson(vector<vector<string> > &respPeople,
                             vector<HumdrumLine *> &references,
                             const string &key, const string &role) {
  for (int i = 0; i < (int)references.size(); i++) {
...
void HumdrumInput::insertTitle(pugi::xml_node &titleStmt,
                               const vector<HumdrumLine *> &references) {
  string key;
  string value;
  bool lang;
  bool plang;
...
void HumdrumInput::prepareTimeSigDur(void) {
  ...
  for (i = 0; i < infile.getLineCount(); i++) {
    ...
    if (sscanf(infile[i].token(0)->c_str(), "*M%d/%d%%%d", &top, &bot, &bot2) ==
        3) {
      // deal with triplet-whole note beats later
    } else if (sscanf(infile[i].token(0)->c_str(), "*M%d/%d", &top, &bot) ==
               2) {
      curdur = top;
      if (bot == 0) { // breve
        curdur *= 2;
...
void HumdrumInput::setKeySig(StaffDef *part, const string &keysig) {
  ...
  } else if (!bb && !eb && !ab && !db && !gb && !cb && !fb && !fs && !cs &&
             !gs && !ds && !as && !es && !bs) {
...
void HumdrumInput::setStaff(ELEMENT element, int staffnum) {
...
bool HumdrumInput::convertMeasureStaves(int startline, int endline) {
  ...
  for (i = 0; i < (int)kernstarts.size(); i++) {
    ...
    status &= convertMeasureStaff(kernstarts[i]->getTrack(), startline, endline,
                                  i + 1, layers[i]);
    if (!status) {
      break;
    }
...
bool HumdrumInput::convertMeasureStaff(int track, int startline, int endline,
...
bool HumdrumInput::convertStaffLayer(int track, int startline, int endline,
                                     int layerindex) {
  m_layer = new Layer();
...
void HumdrumInput::printGroupInfo(vector<humaux::HumdrumBeamAndTuplet> &tg,
                                  const vector<HTp> &layerdata) {
  if (layerdata.size() != tg.size()) {
    cerr << "LAYER SIZE = " << layerdata.size() << "\tTGSIZE" << tg.size()
         << endl;
    return;
  }
...
void HumdrumInput::handleGroupStarts(const humaux::HumdrumBeamAndTuplet &tg,
                                     vector<string> &elements,
                                     vector<void *> &pointers, HTp token) {
  vector<humaux::StaffStateVariables> &ss = m_staffstates;
  ...
  if (tg.beamstart && tg.tupletstart) {
    if (tg.priority == 'T') {
      insertTuplet(elements, pointers, tg, token,
                   ss[staffindex].suppress_beam_tuplet);
    ...
    insertTuplet(elements, pointers, tg, token,
                 ss[staffindex].suppress_bracket_tuplet);
  }
...
void HumdrumInput::handleGroupEnds(const humaux::HumdrumBeamAndTuplet &tg,
                                   vector<string> &elements,
...
bool HumdrumInput::fillContentsOfLayer(int track, int startline, int endline,
                                       int layerindex) {
  int i;
  HumdrumFile &infile = m_infile;
  ...
  HumNum endtime =
      infile[endline].getDurationFromStart() + infile[endline].getDuration();
...
void HumdrumInput::addSpace(vector<string> &elements, vector<void *> &pointers,
                            HumNum duration) {
  while (duration > 0) {
...
void HumdrumInput::processDynamics(HTp token, int staffindex) {
  ...
  for (int i = startfield; i < line->getFieldCount(); i++) {
    ...
    if (!(line->token(i)->isDataType("**dynam") ||
          line->token(i)->isDataType("**dyn"))) {
      continue;
    }
...
template <class ELEMENT>
void HumdrumInput::addTextElement(ELEMENT *element, const string &content) {
  Text *text = new Text;
...
void HumdrumInput::insertClefElement(vector<string> &elements,
                                     vector<void *> pointers, HTp token) {
  Clef *clef = new Clef;
...
void HumdrumInput::analyzeLayerBeams(vector<int> &beamnum,
                                     vector<int> &gbeamnum,
                                     const vector<HTp> &layerdata) {
...
void HumdrumInput::insertTuplet(vector<string> &elements,
                                vector<void *> &pointers,
                                const humaux::HumdrumBeamAndTuplet &tg,
                                HTp token, bool suppress) {
  vector<humaux::StaffStateVariables> &ss = m_staffstates;
...
void HumdrumInput::insertBeam(vector<string> &elements,
                              vector<void *> &pointers,
                              const humaux::HumdrumBeamAndTuplet &tg) {
  Beam *beam = new Beam;
...
void HumdrumInput::insertGBeam(vector<string> &elements,
                               vector<void *> &pointers,
                               const humaux::HumdrumBeamAndTuplet &tg) {
  Beam *gbeam = new Beam;
...
void HumdrumInput::removeBeam(vector<string> &elements,
                              vector<void *> &pointers) {
...
void HumdrumInput::removeGBeam(vector<string> &elements,
                               vector<void *> &pointers) {
...
void HumdrumInput::removeTuplet(vector<string> &elements,
                                vector<void *> &pointers) {
...
void HumdrumInput::prepareBeamAndTupletGroups(
    const vector<hum::HTp> &layerdata,
    vector<humaux::HumdrumBeamAndTuplet> &tg) {
  ...
  for (i = 0; i < (int)dotlessdur.size(); i++) {
    dotlessdur[i] =
        removeFactorsOfTwo(dotlessdur[i], twocounttop[i], twocountbot[i]);
  ...
  for (i = 0; i < (int)beamdur.size(); i++) {
    beamdur[i] =
        dursum[beamends[i]] - dursum[beamstarts[i]] + fulldur[beamends[i]];
  }
...
HumNum HumdrumInput::removeFactorsOfTwo(HumNum value, int &tcount,
                                        int &bcount) {
...
void HumdrumInput::getTimingInformation(vector<HumNum> &prespace,
                                        vector<HTp> &layerdata,
                                        HumNum layerstarttime,
                                        HumNum layerendtime) {
...
bool HumdrumInput::hasFullMeasureRest(vector<HTp> &layerdata, HumNum timesigdur,
                                      HumNum measuredur) {
...
void HumdrumInput::appendElement(PARENT parent, CHILD child) {
...
void HumdrumInput::appendElement(const vector<string> &name,
                                 const vector<void *> &pointers, CHILD child) {
  if (name.back() == "beam") {
...
void HumdrumInput::convertNote(Note *note, HTp token, int staffindex,
                               int subtoken) {
  vector<humaux::StaffStateVariables> &ss = m_staffstates;
  ...
  if (showInAccidGes) {
    ...
    case +1:
      ...
      break;
    case 0:
      note->SetAccidGes(ACCIDENTAL_IMPLICIT_n);
      break;
    case -1:
...
void HumdrumInput::addCautionaryAccidental(Note *note, HTp token, int acount) {
  ...
  switch (acount) {
  case +3:
    accid->SetAccid(ACCIDENTAL_EXPLICIT_ts);
    break;
  case +2:
    accid->SetAccid(ACCIDENTAL_EXPLICIT_x);
    break;
  case +1:
    accid->SetAccid(ACCIDENTAL_EXPLICIT_s);
    break;
  case 0:
    accid->SetAccid(ACCIDENTAL_EXPLICIT_n);
    break;
  case -1:
    accid->SetAccid(ACCIDENTAL_EXPLICIT_f);
    break;
  case -2:
    accid->SetAccid(ACCIDENTAL_EXPLICIT_ff);
    break;
  case -3:
    accid->SetAccid(ACCIDENTAL_EXPLICIT_tf);
    break;
  }
}
...
template <class ELEMENT>
void HumdrumInput::convertRhythm(ELEMENT element, HTp token, int subtoken) {
  string tstring;
  if (subtoken < 0) {
    tstring = *token;
  } else {
  ...
  tstring.erase(std::remove(tstring.begin(), tstring.end(), 'q'),
                tstring.end());
  ...
  if (dur.isInteger()) {
    ...
    case 4:
      ...
      break;
    case 8:
      element->SetDur(DURATION_maxima);
      break;
    }
    ...
    case 512:
      ...
      break;
    case 1024:
      element->SetDur(DURATION_1024);
      break;
    case 2048:
      element->SetDur(DURATION_2048);
      break;
    }
  }
}
...
void HumdrumInput::printNoteArticulations(Note *note, HTp token,
...
void HumdrumInput::processTieStart(Note *note, HTp token,
                                   const string &tstring) {
  vector<humaux::StaffStateVariables> &ss = m_staffstates;
  ...
  ss[rtrack].ties.back().setStart(noteuuid, m_measure, cl, tstring, pitch,
                                  timestamp, endtime);
}
...
void HumdrumInput::processTieEnd(Note *note, HTp token, const string &tstring) {
  ...
  for (auto it = ss[staffnum].ties.begin(); it != ss[staffnum].ties.end();
       it++) {
...
template <class ELEMENT>
HumNum HumdrumInput::setDuration(ELEMENT element, HumNum duration) {
  if (duration == 3) {
    element->SetDur(DURATION_2);
    element->SetDots(1);
    return duration;
    ...
    return duration;
             ...
             (duration.getDenominator() == 8)) {
    element->SetDur(DURATION_32);
    return duration;
  } else if ((duration.getNumerator() == 1) &&
             (duration.getDenominator() == 16)) {
    element->SetDur(DURATION_64);
    return duration;
  } else if ((duration.getNumerator() == 1) &&
             (duration.getDenominator() == 32)) {
    element->SetDur(DURATION_128);
    return duration;
  } else if ((duration.getNumerator() == 1) &&
             (duration.getDenominator() == 64)) {
    element->SetDur(DURATION_256);
    return duration;
  }
  if (duration > 4) {
...
bool HumdrumInput::emptyMeasures(void) { return false; }
...
void HumdrumInput::setLocationId(Object *object, int lineindex, int fieldindex,
                                 int subtokenindex) {
