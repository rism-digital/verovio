...
//////////////////////////////
//
// HumdrumInput::ReadHumdrum -- convert a HumdrumFile structure into an MEI
//   structure.  Returns false if there was an error in the conversion process.
//

bool HumdrumInput::ReadHumdrum(hum::HumdrumFile& infile) {
...
/*
	...
	for (pugi::xpath_node_set::const_iterator it = partListChildren.begin(); it != partListChildren.end(); ++it) {
		...
		if (IsElement(xpathNode.node(), "part-group")) {
			...
			int nbStaves
				= ReadHumdrumPartAttributesAsStaffDef(partFirstMeasureAttributes.node(), partStaffGrp, staffOffset);
			...
			ReadHumdrumPart(part.node(), system, nbStaves, staffOffset);
...
bool HumdrumInput::HasAttributeWithValue(hum::HumdrumFile& node, std::string attribute, std::string value)
{
	assert(node);
...
bool HumdrumInput::IsElement(hum::HumdrumFile& node, std::string name)
{
	assert(node);
...
bool HumdrumInput::HasContentWithValue(hum::HumdrumFile& node, std::string value)
{
	assert(node);
...
bool HumdrumInput::HasContent(hum::HumdrumFile& node)
{
	assert(node);
...
std::string HumdrumInput::GetAttributeValue(hum::HumdrumFile& node, std::string attribute)
{
	assert(node);
...
std::string HumdrumInput::GetContent(hum::HumdrumFile& node)
{
	assert(node);
...
std::string HumdrumInput::GetContentOfChild(hum::HumdrumFile& node, std::string child)
{
...
void HumdrumInput::AddMeasure(System *system, Measure *measure, int i)
{
	assert(system);
	assert(i >= 0);
...
void HumdrumInput::AddLayerElement(Layer *layer, LayerElement *element)
{
	assert(layer);
	assert(element);
...
Layer *HumdrumInput::SelectLayer(hum::HumdrumFile& node, vrv::Measure *measure)
{
	int staffNb = 1;
...
Layer *HumdrumInput::SelectLayer(int staffNb, vrv::Measure *measure)
{
	staffNb--;
	Staff *staff = dynamic_cast<Staff *>(measure->GetChild(staffNb));
	assert(staff);
...
Layer *HumdrumInput::SelectLayer(int layerNb, Staff *staff)
{
	Layer *layer = NULL;
...
void HumdrumInput::RemoveLastFromStack(ClassId classId)
{
...
void HumdrumInput::OpenTie(Staff *staff, Layer *layer, Note *note, Tie *tie)
{
	tie->SetStartid(note->GetUuid());
	hum::OpenTie openTie(staff->GetN(), layer->GetN(), note->GetPname(), note->GetOct());
	m_tieStack.push_back(std::make_pair(tie, openTie));
}

void HumdrumInput::CloseTie(Staff *staff, Layer *layer, Note *note, bool isClosingTie)
{
	std::vector<std::pair<Tie *, hum::OpenTie> >::iterator iter;
	for (iter = m_tieStack.begin(); iter != m_tieStack.end(); iter++) {
		if ((iter->second.m_staffN == staff->GetN()) && (iter->second.m_layerN == layer->GetN())
			...
			if (!isClosingTie) {
				LogWarning("Closing tie for note '%s' even thought tie /tie@[type='stop'] is missing in the Humdrum",
					note->GetUuid().c_str());
			}
			return;
		}
...
void HumdrumInput::OpenSlur(Staff *staff, Layer *layer, int number, LayerElement *element, Slur *slur)
{
	slur->SetStartid(element->GetUuid());
	hum::OpenSlur openSlur(staff->GetN(), layer->GetN(), number);
	m_slurStack.push_back(std::make_pair(slur, openSlur));
}

void HumdrumInput::CloseSlur(Staff *staff, Layer *layer, int number, LayerElement *element)
{
	std::vector<std::pair<Slur *, hum::OpenSlur> >::iterator iter;
...
int HumdrumInput::ReadHumdrumPartAttributesAsStaffDef(hum::HumdrumFile& node, StaffGrp *staffGrp, int staffOffset)
{
	assert(node);
	assert(staffGrp);
...
bool HumdrumInput::ReadHumdrumPart(hum::HumdrumFile& node, System *system, int nbStaves, int staffOffset)
{
	assert(node);
	assert(system);
	...
	for (pugi::xpath_node_set::const_iterator it = measures.begin(); it != measures.end(); ++it) {
		...
		ReadHumdrumMeasure(xmlMeasure.node(), measure, nbStaves, staffOffset);
...
bool HumdrumInput::ReadHumdrumMeasure(hum::HumdrumFile& node, Measure *measure, int nbStaves, int staffOffset)
{
	assert(node);
	assert(measure);
	...
	for (pugi::xml_node::iterator it = node.begin(); it != node.end(); ++it) {
		if (IsElement(*it, "attributes")) {
			ReadHumdrumAttributes(*it, measure, measureNb);
		}
		else if (IsElement(*it, "barline")) {
			ReadHumdrumBarLine(*it, measure, measureNb);
		}
		else if (IsElement(*it, "note")) {
			ReadHumdrumNote(*it, measure, measureNb);
		}
	}
...
void HumdrumInput::ReadHumdrumAttributes(hum::HumdrumFile& node, Measure *measure, int measureNb)
{
...
void HumdrumInput::ReadHumdrumBackup(hum::HumdrumFile& node, Measure *measure, int measureNb)
{
}

void HumdrumInput::ReadHumdrumBarLine(hum::HumdrumFile& node, Measure *measure, int measureNb)
{
...
void HumdrumInput::ReadHumdrumForward(hum::HumdrumFile& node, Measure *measure, int measureNb)
{
}

void HumdrumInput::ReadHumdrumNote(hum::HumdrumFile& node, Measure *measure, int measureNb)
{
	assert(node);
	assert(measure);
...
data_ACCIDENTAL_EXPLICIT HumdrumInput::ConvertAccidentalToAccid(std::string value)
{
...
data_ACCIDENTAL_EXPLICIT HumdrumInput::ConvertAlterToAccid(std::string value)
{
...
data_DURATION HumdrumInput::ConvertTypeToDur(std::string value)
{
...
data_PITCHNAME HumdrumInput::ConvertStepToPitchName(std::string value)
{
