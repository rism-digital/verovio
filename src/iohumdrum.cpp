...
bool HumdrumInput::ImportString(const std::string humdrum)
{
    try {
        m_doc->Reset(Raw);
        pugi::xml_document xmlDoc;
        xmlDoc.load(humdrum.c_str());
        pugi::xml_node root = xmlDoc.first_child();
        return ReadHumdrum(root);
    }
    catch (char *str) {
...
bool HumdrumInput::ReadHumdrum(hum::HumdrumFile& root)
{
    assert(root);
