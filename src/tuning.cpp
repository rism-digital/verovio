...
int Tuning::CalcPitchNumber(int course, int fret, data_NOTATIONTYPE notationType)
{
    ...
    Course* const courseTuning{vrv_cast<Course*>(FindDescendantByComparison(&cnc))};
    ...
    if (courseTuning && courseTuning->HasPname() && courseTuning->HasOct()) {
        ...
        const int alter{(courseTuning->HasAccid())
            ? (courseTuning->GetAccid() == ACCIDENTAL_WRITTEN_s)
                ? 1
                : (courseTuning->GetAccid() == ACCIDENTAL_WRITTEN_f)
                ? -1
                : 0
            : 0};
    ...
    return (course > 0 && course <= pitchSize)
        ? pitch[course - 1] + fret
        : 0;
}
