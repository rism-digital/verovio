...
void KeySig::FilterList(ArrayOfObjects *childList)
{
    ...
    for (auto &child : *childList) {
        KeyAccid *keyAccid = static_cast<KeyAccid *>(child);
        assert(keyAccid);
...
data_ACCIDENTAL_WRITTEN KeySig::GetAccidType()
{
    ...
    if (childList->size() > 0) {
        ...
        KeyAccid *keyAccid = static_cast<KeyAccid *>(childList->at(0));
        assert(keyAccid);
...
std::wstring KeySig::GetKeyAccidStrAt(int pos, data_ACCIDENTAL_WRITTEN &accid, data_PITCHNAME &pname)
{
    ...
    if (childList->size() > 0) {
        ...
        KeyAccid *keyAccid = static_cast<KeyAccid *>(childList->at(pos));
        assert(keyAccid);
