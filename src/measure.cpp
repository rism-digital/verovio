...
bool Measure::IsSupportedChild(Object *child)
{
    if (child->IsControlElement()) {
        ...
        Staff *staff = static_cast<Staff *>(child);
        assert(staff);
...
int Measure::GetDrawingOverflow()
{
    ...
    adjustXOverflowParams.m_currentSystem = static_cast<System *>(this->GetFirstAncestor(SYSTEM));
...
{
    UnsetCurrentScoreDefParams *params = static_cast<UnsetCurrentScoreDefParams *>(functorParams);
    assert(params);
...
int Measure::CastOffSystems(FunctorParams *functorParams)
{
    ...
    if (params->m_currentSystem->GetChildCount() > 0) {
        ...
        if (overflow > (params->m_doc->GetDrawingUnit(100) * 5)) {
            Measure *measure = static_cast<Measure *>(params->m_contentSystem->Relinquish(this->GetIdx()));
            assert(measure);
        ...
        // Break it if necessary
            ...
            for (Object *oneOfPendingObjects : params->m_pendingObjects) {
                if (oneOfPendingObjects->Is(MEASURE)) {
                    Measure *firstPendingMesure = static_cast<Measure *>(oneOfPendingObjects);
...
{
    CalcMaxMeasureDurationParams *params = static_cast<CalcMaxMeasureDurationParams *>(functorParams);
    assert(params);
