...
bool LayerElement::IsGraceNote()
{
    ...
    if (this->Is(NOTE)) {
        Note const *note = static_cast<Note const *>(this);
        assert(note);
        ...
        Chord const *chord = static_cast<Chord const *>(this);
        assert(chord);
        return (chord->HasGrace());
    }
...
int LayerElement::GetDrawingRadius(Doc *doc, bool isInLigature)
{
    ...
    assert(staff);
    if (this->Is(NOTE)) {
        Note *note = static_cast<Note *>(this);
        assert(note);
    ...
    Mensur *mensur, MeterSig *meterSig, bool notGraceOnly, data_NOTATIONTYPE notationType)
{
    ...
    if (this->HasSameasLink() && this->GetSameasLink()->IsLayerElement()) {
        LayerElement *sameas = static_cast<LayerElement *>(this->GetSameasLink());
        assert(sameas);
        ...
        BeatRpt *beatRpt = static_cast<BeatRpt *>(this);
        assert(beatRpt);
        int meterUnit = 4;
        ...
        TimestampAttr *timestampAttr = static_cast<TimestampAttr *>(this);
    ...
    Mensur *mensur, MeterSig *meterSig, bool notGraceOnly, data_NOTATIONTYPE notationType)
{
    ...
    for (auto child : *this->GetChildren()) {
        ...
        LayerElement *element = static_cast<LayerElement *>(child);
        assert(element);
...
int LayerElement::AlignHorizontally(FunctorParams *functorParams)
{
    ...
    // We do not align these (formely container). Any other?
        ...
        if ((this->GetScoreDefRole() == SCOREDEF_SYSTEM) || (this->GetScoreDefRole() == SCOREDEF_INTERMEDIATE))
            ...
            params->m_currentMensur = static_cast<Mensur *>(this);
        ...
        if ((this->GetScoreDefRole() == SCOREDEF_SYSTEM) || (this->GetScoreDefRole() == SCOREDEF_INTERMEDIATE))
            ...
            params->m_currentMeterSig = static_cast<MeterSig *>(this);
        ...
        Note *note = static_cast<Note *>(this->GetFirstAncestor(NOTE));
        assert(note);
...
{
    SetAlignmentPitchPosParams *params = static_cast<SetAlignmentPitchPosParams *>(functorParams);
    assert(params);

    if (this->IsScoreDefElement()) return FUNCTOR_SIBLINGS;

    LayerElement *layerElementY = this;
    Staff *staffY = static_cast<Staff *>(this->GetFirstAncestor(STAFF));
    assert(staffY);
    Layer *layerY = static_cast<Layer *>(this->GetFirstAncestor(LAYER));
    assert(layerY);
    ...
    if (this->Is(ACCID)) {
        Accid *accid = static_cast<Accid *>(this);
        assert(accid);
        ...
        Custos *custos = static_cast<Custos *>(this);
        assert(custos);
        int loc = 0;
        if (custos->HasPname()) {
        ...
        MRest *mRest = static_cast<MRest *>(this);
        assert(mRest);
        int loc = 0;
        ...
        // Automatically calculate rest position
            ...
            if (hasMultipleLayer) {
                Layer *firstLayer = static_cast<Layer *>(staffY->FindDescendantByType(LAYER));
                assert(firstLayer);
        ...
        Rest *rest = static_cast<Rest *>(this);
        assert(rest);
        int loc = 0;
        ...
        // Automatically calculate rest position
            ...
            if (beam) {
                ...
                for (; rit != beamList->rend(); ++rit) {
                    LayerElement *layerElement = static_cast<LayerElement *>(*rit);
                    assert(layerElement);
                ...
                for (; it != beamList->end(); ++it) {
                    LayerElement *layerElement = static_cast<LayerElement *>(*it);
                    assert(layerElement);
                    if (layerElement->Is(NOTE)) {
...
{
    AdjustLayersParams *params = static_cast<AdjustLayersParams *>(functorParams);
    assert(params);
    ...
    if (this->Is({ NOTE, DOTS })) {
        ...
        for (iter = params->m_previous.begin(); iter != params->m_previous.end(); ++iter) {
            ...
            if (this->Is(NOTE) && (*iter)->Is(NOTE)) {
                assert(params->m_currentNote);
                Note *previousNote = static_cast<Note *>(*iter);
                assert(previousNote);
...
{
    AdjustGraceXPosParams *params = static_cast<AdjustGraceXPosParams *>(functorParams);
    assert(params);
...
{
    AdjustXPosParams *params = static_cast<AdjustXPosParams *>(functorParams);
    assert(params);
...
int LayerElement::PrepareDrawingCueSize(FunctorParams *functorParams)
{
    ...
    // For accid, look at the parent if @func="edit" or otherwise to the parent note
        ...
        Accid const *accid = static_cast<Accid *>(this);
        assert(accid);
...
{
    PrepareCrossStaffParams *params = static_cast<PrepareCrossStaffParams *>(functorParams);
    assert(params);
    ...
    Staff *parentStaff = static_cast<Staff *>(this->GetFirstAncestor(STAFF));
    assert(parentStaff);
...
{
    FindSpannedLayerElementsParams *params = static_cast<FindSpannedLayerElementsParams *>(functorParams);
    assert(params);
...
int LayerElement::CalcOnsetOffset(FunctorParams *functorParams)
{
    ...
    if (element->Is(REST) || element->Is(SPACE)) {
        ...
        BeatRpt *rpt = static_cast<BeatRpt *>(element);
        assert(rpt);
