...
{
    ScoreDef *scoreDef = this->GetLastVisibleScoreDef();
    if (scoreDef->FindDescendantByType(PGFOOT)) {
        return false;
    }
...
ScoreDef *Doc::GetLastVisibleScoreDef()
{
    if (m_visibleScores.empty()) {
        this->CollectVisibleScores();
    ...
    return m_visibleScores.back()->GetScoreDef();
}
