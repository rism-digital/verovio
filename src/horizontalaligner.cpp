...
Alignment *HorizontalAligner::SearchAlignmentAtTime(double time, AlignmentType type, int &idx)
{
    ...
    for (i = 0; i < GetAlignmentCount(); ++i) {
        alignment = static_cast<Alignment *>(m_children.at(i));
        assert(alignment);
...
void MeasureAligner::AdjustProportionally(const ArrayOfAdjustmentTuples &adjustments)
{
    ...
    for (iter = adjustments.begin(); iter != adjustments.end(); ++iter) {
        ...
        for (alignIter = m_children.begin(); alignIter != m_children.end(); ++alignIter) {
            Alignment *current = static_cast<Alignment *>(*alignIter);
            assert(current);
...
void MeasureAligner::PushAlignmentsRight()
{
    ...
    for (riter = m_children.rbegin(); riter != m_children.rend(); ++riter) {
        Alignment *current = static_cast<Alignment *>(*riter);
        assert(current);
...
void MeasureAligner::AdjustGraceNoteSpacing(Doc *doc, Alignment *alignment, int staffN)
{
    ...
    Measure *measure = static_cast<Measure *>(this->GetParent());
    assert(measure);
    ...
    for (riter = m_children.rbegin(); riter != m_children.rend(); ++riter) {
        ...
        rightAlignment = static_cast<Alignment *>(*riter);
...
void GraceAligner::AlignStack()
{
    ...
    for (i = (int)m_graceStack.size(); i > 0; i--) {
        LayerElement *element = static_cast<LayerElement *>(m_graceStack.at(i - 1));
        assert(element);
        ...
        for (auto &child : children) {
            ...
            LayerElement *childElement = static_cast<LayerElement *>(child);
            assert(childElement);
...
void GraceAligner::SetGraceAligmentXPos(Doc *doc)
{
    ...
    for (childrenIter = m_children.rbegin(); childrenIter != m_children.rend(); ++childrenIter) {
        Alignment *alignment = static_cast<Alignment *>(*childrenIter);
        assert(alignment);
...
TimestampAttr *TimestampAligner::GetTimestampAtTime(double time)
{
    ...
    for (i = 0; i < GetChildCount(); ++i) {
        timestampAttr = static_cast<TimestampAttr *>(m_children.at(i));
...
{
    SetAlignmentXPosParams *params = static_cast<SetAlignmentXPosParams *>(functorParams);
    assert(params);
...
{
    AdjustArpegParams *params = static_cast<AdjustArpegParams *>(functorParams);
    assert(params);
...
int Alignment::AdjustGraceXPos(FunctorParams *functorParams)
{
    ...
    if (!params->m_isGraceAlignment) {
        ...
        MeasureAligner *measureAligner = static_cast<MeasureAligner *>(this->GetFirstAncestor(MEASURE_ALIGNER));
...
{
    AdjustAccidXParams *params = static_cast<AdjustAccidXParams *>(functorParams);
    assert(params);
...
int AlignmentReference::AdjustAccidX(FunctorParams *functorParams)
{
    ...
    for (iter = m_accidSpace.begin(); iter != m_accidSpace.end() - 1; ++iter) {
        Note *note = static_cast<Note *>((*iter)->GetFirstAncestor(NOTE));
        assert(note);
        if (!note) continue;
        for (octaveIter = iter + 1; octaveIter != m_accidSpace.end(); ++octaveIter) {
            Note *octave = static_cast<Note *>((*octaveIter)->GetFirstAncestor(NOTE));
            assert(octave);
            if (!octave) continue;
...
{
    Alignment *alignment = static_cast<Alignment *>(this->GetParent());
    assert(alignment);
