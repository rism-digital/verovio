...
void Arpeg::GetDrawingTopBottomNotes(Note *&top, Note *&bottom)
{
    ...
    if (front == back) {
        ...
        Chord *chord = static_cast<Chord *>(front);
        assert(chord);
    ...
    if (front->Is(CHORD)) {
        chord1 = static_cast<Chord *>(front);
        assert(chord1);
    }
    else {
        note1 = static_cast<Note *>(front);
        assert(note1);
    }
    if (back->Is(CHORD)) {
        chord2 = static_cast<Chord *>(back);
        assert(chord2);
    }
    else {
        note2 = static_cast<Note *>(back);
        assert(note2);
    }
...
int Arpeg::AdjustArpeg(FunctorParams *functorParams)
{
    ...
    Staff *topStaff = static_cast<Staff *>(topNote->GetFirstAncestor(STAFF));
    assert(topStaff);

    Staff *bottomStaff = static_cast<Staff *>(bottomNote->GetFirstAncestor(STAFF));
    assert(bottomStaff);
