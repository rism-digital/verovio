...
LayerElement *Layer::GetAtPos(int x)
{
    ...
    LayerElement *element = static_cast<LayerElement *>(first);
    assert(element);
    ...
    while ((next = this->GetNext())) {
        ...
        LayerElement *nextLayerElement = static_cast<LayerElement *>(next);
...
Clef *Layer::GetClef(LayerElement *test)
{
    ...
    if (testObject && testObject->Is(CLEF)) {
        Clef *clef = static_cast<Clef *>(testObject);
        assert(clef);
        return clef;
    }
...
data_STEMDIRECTION Layer::GetDrawingStemDir(const ArrayOfBeamElementCoords *coords)
{
    ...
    Staff *staff = static_cast<Staff *>(first->GetFirstAncestor(STAFF));
    assert(staff);
...
int Layer::AlignHorizontallyEnd(FunctorParams *functorParams)
{
    ...
    for (i = 0; i < params->m_measureAligner->GetChildCount(); ++i) {
        Alignment *alignment = static_cast<Alignment *>(params->m_measureAligner->GetChild(i));
        assert(alignment);
