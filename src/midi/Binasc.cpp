...
int Binasc::writeToBinary(const std::string &outfile, const std::string &infile)
{
    std::ifstream input;
    input.open(infile.c_str());
    if (!input.is_open()) {
        std::cerr << "Cannot open " << infile << " for reading in binasc." << std::endl;
        return 0;
    }
    ...
    if (!output.is_open()) {
        std::cerr << "Cannot open " << outfile << " for reading in binasc." << std::endl;
        return 0;
    }
...
int Binasc::processLine(std::ostream &out, const std::string &input, int lineCount)
{
    int status = 1;
    int i = 0;
...
int Binasc::getWord(std::string &word, const std::string &input, const std::string &terminators, int index)
{
    word.resize(0);
    int i = index;
    int escape = 0;
...
int Binasc::readMidiEvent(std::ostream &out, std::istream &infile, int &trackbytes, int &command)
{
...
int Binasc::processDecimalWord(std::ostream &out, const std::string &word, int lineNum)
{
    ...
    for (i = 0; i < length; i++) {
        switch (word[i]) {
            ...
            case '-':
                if (signIndex != -1) {
                    std::cerr << "Error on line " << lineNum << " at token: " << word << std::endl;
                    std::cerr << "cannot have more than two minus signs in number" << std::endl;
                    return 0;
                }
                else {
                    signIndex = i;
            ...
            case '9':
                if (quoteIndex == -1) {
                    std::cerr << "Error on line " << lineNum << " at token: " << word << std::endl;
                    std::cerr << "cannot have numbers before quote in decimal number" << std::endl;
                    return 0;
                }
                break;
            default:
    ...
    switch (byteCount) {
        ...
        case 3: {
            if (signIndex != -1) {
                std::cerr << "Error on line " << lineNum << " at token: " << word << std::endl;
                std::cerr << "negative decimal numbers cannot be stored in 3 bytes" << std::endl;
                return 0;
            }
            ulong tempLong = (ulong)atoi(&word[quoteIndex + 1]);
...
int Binasc::processHexWord(std::ostream &out, const std::string &word, int lineNum)
{
...
int Binasc::processStringWord(std::ostream &out, const std::string &word, int /* lineNum */)
{
    out << word;
    return 1;
}
...
int Binasc::processAsciiWord(std::ostream &out, const std::string &word, int lineNum)
{
    int length = (int)word.size();
    uchar outputByte;
    ...
    if (length > 2) {
        std::cerr << "Error on line " << lineNum << " at token: " << word << std::endl;
        std::cerr << "character byte word is too long -- specify only one character" << std::endl;
        return 0;
    }
...
int Binasc::processBinaryWord(std::ostream &out, const std::string &word, int lineNum)
{
    int length = (int)word.size(); // length of ascii binary number
...
int Binasc::processVlvWord(std::ostream &out, const std::string &word, int lineNum)
{
    if (word.size() < 2) {
        std::cerr << "Error on line: " << lineNum << ": 'v' needs to be followed immediately by a decimal digit"
                  << std::endl;
        return 0;
    }
    if (!isdigit(word[1])) {
...
int Binasc::processMidiTempoWord(std::ostream &out, const std::string &word, int lineNum)
{
    if (word.size() < 2) {
        std::cerr << "Error on line: " << lineNum << ": 't' needs to be followed immediately by "
...
int Binasc::processMidiPitchBendWord(std::ostream &out, const std::string &word, int lineNum)
{
    if (word.size() < 2) {
        std::cerr << "Error on line: " << lineNum << ": 'p' needs to be followed immediately by "
                  << "a floating-point number" << std::endl;
        return 0;
    }
    if (!(isdigit(word[1]) || word[1] == '.' || word[1] == '-' || word[1] == '+')) {
