...
int GABCInput::ProcessSuffix(const std::string &music, int currentIndex, Nc *nc, Nc *previousNC)
{
    ...
    switch (nextChar) {
        case 'V':
            // grule virga_left. Aquitanian sources use a north-east tilt instead of
        ...
        case '\'':
            processedChars++;
            AddEpisema(nc, episemaVis_FORM_v, EVENTREL_below);
            break;

        case '_':
            processedChars++;
            AddEpisema(nc, episemaVis_FORM_h, EVENTREL_above);
            break;
...
bool GABCInput::Import(const std::string &gabc)
{
    ...
    m_pendingNcType.clear(); // reset cross-token accumulator from any prior Import call
    m_doc->Reset();
    m_doc->SetType(Raw);
    ...
    staffDef->SetN(1);
    // The number of staff lines comes from the gabcStaffLines CLI option, which mirrors
    ...
    // grule clef_flat — attach a one-accidental KeySig to the StaffDef when a `cb<n>`
