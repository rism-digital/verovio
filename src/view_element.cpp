...
void View::DrawStem( DeviceContext *dc, LayerElement *object, Staff *staff, data_STEMDIRECTION dir, int radius, int xn, int originY, int heightY)
{
    ...
    if (drawingCueSize) {
        baseStem = baseStem * m_doc->m_style->m_graceNum / m_doc->m_style->m_graceDen;
        flagStemHeight = flagStemHeight * m_doc->m_style->m_graceNum / m_doc->m_style->m_graceDen;
    }
    ...
    if (drawingDur > DUR_4) {
        ...
        int shortener = (drawingCueSize) ?
            m_doc->GetDrawingUnit(staffSize) * m_doc->m_style->m_graceNum / m_doc->m_style->m_graceDen :
    ...
    if (dir == STEMDIRECTION_up) {
        ...
        if (drawingDur > DUR_4) {
            DrawSmuflCode( dc, x2 - m_doc->m_style->m_stemWidth, y2, SMUFL_E240_flag8thUp, staff->staffSize, drawingCueSize );
            for (int i=0; i < nbFlags; i++)
                DrawSmuflCode( dc, x2 - m_doc->m_style->m_stemWidth, y2 - (i + 1) * flagStemHeight, SMUFL_E240_flag8thUp,  staff->staffSize, drawingCueSize );
        }
    }
    else {
        ...
        if (drawingDur > DUR_4) {
            DrawSmuflCode( dc, x2 , y2, SMUFL_E241_flag8thDown , staff->staffSize, drawingCueSize );
            for (int i=0; i < nbFlags; i++)
...
{	
    ...
    if (drawingDur > DUR_2)
    {
		x -= m_doc->GetGlyphWidth(SMUFL_E0A3_noteheadHalf, staff->staffSize, drawingCueSize);
    }
    ...
    {	
        ...
        case DUR_1:
        case DUR_2: DrawWholeRest ( dc, x, y, drawingDur, rest->GetDots(), drawingCueSize, staff); break;
        default: DrawQuarterRest( dc, x, y, drawingDur, rest->GetDots(), drawingCueSize, staff);
    }
...
void View::DrawMRest(DeviceContext *dc, LayerElement *element, Layer *layer, Staff *staff, Measure *measure)
{
    ...
    y -= staff->m_drawingLines / 2 * m_doc->GetDrawingDoubleUnit(staff->staffSize) - m_doc->GetDrawingDoubleUnit(staff->staffSize);
...
void View::DrawMultiRest(DeviceContext *dc, LayerElement *element, Layer *layer, Staff *staff, Measure *measure)
{
	...
	length = (m_doc->GetDrawingUnit(staff->staffSize) * 5);
    
    // Position centered in third line
    // it would be m_drawingDoubleUnit * 6.5, or m_drawingDoubleUnit / 2 * 13
	y1 = staff->GetDrawingY() - (m_doc->GetDrawingDoubleUnit(staff->staffSize) / 2) * 5;
    y2 = y1 + m_doc->GetDrawingDoubleUnit(staff->staffSize);
    ...
    int border = m_doc->GetDrawingUnit(staff->staffSize);
...
void View::DrawLongRest ( DeviceContext *dc, int x, int y, Staff *staff)
{
    ...
    y1 = y;
	x1 = x;
	x2 = x + (m_doc->GetDrawingUnit(staff->staffSize) * 2 / 3);
    
    // look if on line or between line
	if ( (y - staff->GetDrawingY()) % m_doc->GetDrawingDoubleUnit(staff->staffSize))
		y1 -= m_doc->GetDrawingUnit(staff->staffSize);
    
	y2 = y1 + m_doc->GetDrawingDoubleUnit(staff->staffSize)*2;
...
{	
    ...
    // lines
    x1 = x - m_doc->GetDrawingUnit(staff->staffSize);
    x2 = x + m_doc->GetDrawingUnit(staff->staffSize);
...
void View::DrawWholeRest ( DeviceContext *dc, int x, int y, int valeur, unsigned char dots, unsigned int smaller, Staff *staff)
{
	...
	if (dots)
		DrawDots( dc, (x2 + m_doc->GetDrawingUnit(staff->staffSize)), y2, dots, staff);
}
...
void View::DrawQuarterRest ( DeviceContext *dc, int x, int y, int valeur, unsigned char dots, unsigned int smaller, Staff *staff)
{
	...
	if (dots)
	{	if (valeur < DUR_16)
			y += m_doc->GetDrawingDoubleUnit(staff->staffSize);
		DrawDots( dc, (x + 2 * m_doc->GetDrawingDoubleUnit(staff->staffSize)), y, dots, staff);
	}
	return;
}
...
{
    return ((y - staff->GetDrawingY()) % m_doc->GetDrawingDoubleUnit(staff->staffSize) == 0 );
}
...
void View::PrepareChordDots ( DeviceContext *dc, Chord *chord, int x, int y, unsigned char dots, Staff *staff )
{
    ...
    int fullUnit = m_doc->GetDrawingUnit(staff->staffSize);
...
void View::DrawDots ( DeviceContext *dc, int x, int y, unsigned char dots, Staff *staff)
{
	int i;
    if ( IsOnStaffLine(y, staff) ) {
        y += m_doc->GetDrawingUnit(staff->staffSize);
    }
	for (i = 0; i < dots; i++) {
        DrawDot ( dc, x, y );
		x += std::max (6, 2 * m_doc->GetDrawingUnit(staff->staffSize));
	}
	return;
}
...
void View::DrawClef( DeviceContext *dc, LayerElement *element, Layer *layer, Staff *staff, Measure *measure )
{
    ...
    if (!cueSize)
        a -= m_doc->GetDrawingUnit(staff->staffSize) * 2;
...
void View::DrawMeterSigFigures( DeviceContext *dc, int x, int y, int num, int numBase, Staff *staff)
{
    ...
    if (numBase)
    {
        ynum = y - (m_doc->GetDrawingUnit(staff->staffSize)*2);
        yden = ynum - (m_doc->GetDrawingDoubleUnit(staff->staffSize)*2);
    }
    else
        ynum = y - (m_doc->GetDrawingUnit(staff->staffSize)*4);
...
void View::DrawMeterSig( DeviceContext *dc, LayerElement *element, Layer *layer, Staff *staff, Measure *measure )
{
    ...
    if ( meterSig->GetSym() == METERSIGN_common ) {
        ...
        x += m_doc->GetDrawingUnit(staff->staffSize) * 5; // step forward because we have a symbol
    }
    ...
    if ( accid->GetFunc() == FUNC_edit ) {
        ...
        if ( note ) {
            if ( note->GetDrawingY() > y ) {
                y = note->GetDrawingY() + m_doc->GetDrawingUnit(staff->staffSize);
            }
            ...
            int radius = m_doc->GetGlyphWidth(SMUFL_E0A3_noteheadHalf, staff->staffSize, accid->m_drawingCueSize);
    ...
    switch (accid->GetAccid())
    {
        ...
        case ACCIDENTAL_EXPLICIT_ss :
            ...
            if (staff->notAnc) {
                    ...
                    DrawSmuflCode ( dc, x, y, symc,  staff->staffSize, accid->m_drawingCueSize );
                    y += 7*m_doc->GetDrawingUnit(staff->staffSize); // LP
            } else {
...
void View::DrawCustos( DeviceContext *dc, LayerElement *element, Layer *layer, Staff *staff, Measure *measure )
{
    ...
    y -= m_doc->GetDrawingUnit(staff->staffSize) - m_doc->GetDrawingUnit(staff->staffSize)/4;  // LP - correction in 2.0.0
...
void View::DrawDot( DeviceContext *dc, LayerElement *element, Layer *layer, Staff *staff, Measure *measure )
{
    ...
    if ( dot->m_drawingNote ) {
        x = dot->m_drawingNote->GetDrawingX() + m_doc->GetDrawingUnit(staff->staffSize)*7/2;
...
void View::DrawSyl( DeviceContext *dc, LayerElement *element, Layer *layer, Staff *staff, Measure *measure )
{
    ...
    syl->SetDrawingX( syl->GetStart()->GetDrawingX() - m_doc->GetDrawingUnit(staff->staffSize) * 2 );
...
void View::DrawAcciaccaturaSlash(DeviceContext *dc, LayerElement *element)
{
    ...
    dc->SetPen(AxBLACK, m_doc->m_style->m_stemWidth, AxSOLID);
    dc->SetBrush( AxBLACK, AxSOLID );
    
    int positionShift = m_doc->GetDrawingUnit(staff->staffSize) * m_doc->m_style->m_graceNum / m_doc->m_style->m_graceDen;
    ...
    if (element->Is() == NOTE) {
        ...
        */
            ...
            if (note->m_embellishment)
                emb_offset = m_doc->GetDrawingDoubleUnit(staff->staffSize);
            ...
            if ((element->GetDrawingY()) < staff->GetDrawingY())
                ...
                y = staff->GetDrawingY() + m_doc->GetDrawingUnit(staff->staffSize) + emb_offset;
            else
                // out of the staff, place the trill above the notehead
                y = (element->GetDrawingY()) + m_doc->GetDrawingUnit(staff->staffSize) + emb_offset;
        ...
        /*
            ...
            if ((element->GetDrawingY()) > (staff->GetDrawingY() - m_doc->m_drawingStaffSize[staff->staffSize]))
                ...
                y = staff->GetDrawingY() - m_doc->m_drawingStaffSize[staff->staffSize] - m_doc->GetDrawingDoubleUnit(staff->staffSize);
            else
                // notehead under staff, set under notehead
                y = (element->GetDrawingY()) - m_doc->GetDrawingDoubleUnit(staff->staffSize);
        ...
        */
        ...
        y = staff->GetDrawingY() + m_doc->GetDrawingDoubleUnit(staff->staffSize);
        DrawSmuflCode ( dc, x, y, SMUFL_E4C0_fermataAbove, staff->staffSize, false );
    }
}
