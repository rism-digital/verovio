...
std::string AttConverter::DivLineToStr(data_DIVLINE data) const
{
    std::string value;
    switch (data) {
        case DIVLINE_minima: value = "minima"; break;
        case DIVLINE_maior: value = "maior"; break;
        case DIVLINE_maxima: value = "maxima"; break;
        case DIVLINE_finalis: value = "finalis"; break;
        
        default:
            LogWarning("Unknown value '%d' for data.DIVLINE", data);
            value = "";
            break;
    }
    return value;
...
data_DIVLINE AttConverter::StrToDivLine(const std::string &value, bool logWarning) const
{
    if (value == "minima") return DIVLINE_minima;
    if (value == "maior") return DIVLINE_maior;
    if (value == "maxima") return DIVLINE_maxima;
    if (value == "finalis") return DIVLINE_finalis;
    
    if (logWarning && !value.empty())
        LogWarning("Unsupported value '%s' for data.DIVLINE", value.c_str());
    return DIVLINE_NONE;
}
