...
{
    m_tabAlign = VERTICALALIGNMENT_NONE;
    m_tabAnchorline = 0;
}

bool AttStaffDefVisTablature::ReadStaffDefVisTablature(pugi::xml_node element, bool removeAttr)
{
    bool hasAttribute = false;
    if (element.attribute("tab.align")) {
        this->SetTabAlign(StrToVerticalalignment(element.attribute("tab.align").value()));
        if (removeAttr) element.remove_attribute("tab.align");
        hasAttribute = true;
    }
    if (element.attribute("tab.anchorline")) {
        this->SetTabAnchorline(StrToInt(element.attribute("tab.anchorline").value()));
...
bool AttStaffDefVisTablature::WriteStaffDefVisTablature(pugi::xml_node element)
{
    bool wroteAttribute = false;
    if (this->HasTabAlign()) {
        element.append_attribute("tab.align") = VerticalalignmentToStr(this->GetTabAlign()).c_str();
        wroteAttribute = true;
    }
    if (this->HasTabAnchorline()) {
        element.append_attribute("tab.anchorline") = IntToStr(this->GetTabAnchorline()).c_str();
        wroteAttribute = true;
    }
    return wroteAttribute;
}

bool AttStaffDefVisTablature::HasTabAlign() const
{
    return (m_tabAlign != VERTICALALIGNMENT_NONE);
}

bool AttStaffDefVisTablature::HasTabAnchorline() const
{
    return (m_tabAnchorline != 0);
}
